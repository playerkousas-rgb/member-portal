...
// ║     MEMBER_{區碼}_APIKEY = 區方提交的 API Key                     ║
// ║     例如區碼 CHW → 設 MEMBER_CHW_APIKEY=ak_xxxxxxxx             ║
...
// ╔══════════════════════════════════════════════════════════════════╗
// ║  單區模式開關（對外隱藏多區能力）                                ║
// ║  MULTI_DISTRICT_MODE = false 時，系統對外只呈現一個區：           ║
// ║    · 自動進入 DEFAULT_DISTRICT_CODE，唔會出現揀區畫面             ║
// ║    · header 唔顯示「地區下拉 / 清除地區」                        ║
// ║    · 「區接入」「使用地區」頁隱藏（導回主控台）                    ║
...
export function withDistrictParam(path: string, code: string | null | undefined): string {
  ...
  if (!MULTI_DISTRICT_MODE) return path;
